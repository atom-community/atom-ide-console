...
export function activate(rawState: ?Object) {
  ...
  _disposables = new UniversalDisposable(
    ...
    atom.commands.add("atom-workspace", "console:clear", () => _getStore().dispatch(Actions.clearRecords())),
    featureConfig.observe("atom-ide-console.maximumMessageCount", (maxMessageCount: any) => {
      _getStore().dispatch(Actions.setMaxMessageCount(maxMessageCount))
    }),
    Observable.combineLatest(
      observableFromSubscribeFunction((cb) => atom.config.observe("editor.fontSize", cb)),
...
export function _getStore(): Store {
  if (_store == null) {
    ...
    _store = createStore(Reducers, initialState, applyMiddleware(createEpicMiddleware(rootEpic)))
  }
  return _store
}
...
export function serialize(): Object {
  ...
  const maximumSerializedMessages: number = (featureConfig.get(MAXIMUM_SERIALIZED_MESSAGES_CONFIG): any)
  const maximumSerializedHistory: number = (featureConfig.get(MAXIMUM_SERIALIZED_HISTORY_CONFIG): any)
  return {
    records: _store
      .getState()
...
function deserializeAppState(rawState: ?Object): AppState {
  return {
    ...
    records: rawState && rawState.records ? List(rawState.records.map(deserializeRecord)) : List(),
    incompleteRecords:
      rawState && rawState.incompleteRecords ? List(rawState.incompleteRecords.map(deserializeRecord)) : List(),
