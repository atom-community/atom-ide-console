 ...
 */
export default function parseText(text: string): Array<string | React.Element<any>> {
  ...
  while (true) {
    ...
    if (match[1] != null) {
      // It's a diff
      const url = toString(featureConfig.get("atom-ide-console.diffUrlPattern"))
      if (url !== "") {
        href = url.replace("%s", matchedText)
      }
    } else if (match[2] != null) {
      // It's a task
      const url = toString(featureConfig.get("atom-ide-console.taskUrlPattern"))
      if (url !== "") {
    ...
    chunks.push(
      ...
      href ? (
        <a key={`r${index}`} href={href} target="_blank" onClick={handleOnClick}>
          {matchedText}
        </a>
      ) : (
        matchedText
