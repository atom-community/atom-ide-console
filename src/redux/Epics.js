 ...
 */
export function registerExecutorEpic(actions: ActionsObservable<Action>, store: Store): Observable<Action> {
 ...
 */
export function executeEpic(actions: ActionsObservable<Action>, store: Store): Observable<Action> {
...
export function trackEpic(actions: ActionsObservable<Action>, store: Store): Observable<empty> {
  return actions
...
export function registerRecordProviderEpic(actions: ActionsObservable<Action>, store: Store): Observable<Action> {
  return actions.ofType(Actions.REGISTER_RECORD_PROVIDER).flatMap((action) => {
    ...
    const statusActions =
      ...
      typeof recordProvider.observeStatus === "function"
        ? observableFromSubscribeFunction(recordProvider.observeStatus).map((status) =>
            Actions.updateStatus(recordProvider.id, status)
          )
        : Observable.empty()
    ...
    return Observable.merge(
      Observable.of(Actions.registerSource({ ...recordProvider, name: recordProvider.id })),
      messageActions,
      statusActions
