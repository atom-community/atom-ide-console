...
function shouldAccumulateRecordCount(recordA: Record, recordB: Record): boolean {
  if (
    String(recordA.sourceId).toLowerCase().includes("debugger") ||
    String(recordB.sourceId).toLowerCase().includes("debugger")
  ) {
    return false
  }
  ...
  const doesDataExist =
    (recordA.expressions && recordA.expressions.length > 0) || (recordB.expressions && recordB.expressions.length > 0)

  const recATags = recordA.tags
  const recBTags = recordB.tags
  const areTagsEqual = (!recATags && !recBTags) || (recATags && recBTags && arrayEqual(recATags, recBTags))
...
export default function accumulateState(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case Actions.RECORD_UPDATED: {
      let { records, incompleteRecords } = state
      const { messageId, appendText, overrideLevel, setComplete } = action.payload
    ...
    case Actions.EXECUTE: {
      ...
      const newHistory =
        state.history[state.history.length - 1] === command ? state.history : state.history.concat(command)
      return {
        ...state,
