...
export default class InputArea extends React.Component<Props, State> {
  ...
  _handleKeyDown = (event: KeyboardEvent): void => {
    ...
    if (event.which === ENTER_KEY_CODE) {
      ...
      (editor.getLineCount() <= 1 || editor.getCursorBufferPosition().row === editor.getLineCount() - 1)
    ) {
      if (this.props.history.length === 0 || isAutocompleteOpen) {
        return
      }
  ...
  render(): React.Node {
    ...
    return (
      <div className="console-input-wrapper" style={{ fontSize: `${this.props.fontSize}px` }}>
        <AtomTextEditor
