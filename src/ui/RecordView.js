...
export default class RecordView extends React.Component<Props> {
  ...
  _renderNestedValueComponent(): React.Element<any> {
    ...
    for (const expression of expressions) {
      if (!expression.hasChildren()) {
        children.push(<SimpleValueComponent hideExpressionName={true} expression={expression} />)
      } else {
        children.push(
    ...
    return children.length <= 1 ? children[0] : <span className="console-multiple-objects">{children}</span>
  }
  ...
  render(): React.Node {
    ...
    const sourceLabel = this.props.showSourceLabel ? (
      <span className={`console-record-source-label ${getHighlightClassName(level)}`}>{sourceName ?? sourceId}</span>
    ) : null
    let renderedTimestamp
    if (timestamp != null) {
      const timestampLabel =
        Date.now() - timestamp > ONE_DAY ? timestamp.toLocaleString() : timestamp.toLocaleTimeString()
      renderedTimestamp = <div className="console-record-timestamp">{timestampLabel}</div>
    }
    return (
      <MeasuredComponent onMeasurementsChanged={this._debouncedMeasureAndNotifyHeight}>
        {/* $FlowFixMe(>=0.53.0) Flow suppress */}
        <div ref={this._handleRecordWrapper} className={classNames} tabIndex="0">
          {icon}
          <div className="console-record-content-wrapper">
            {record.repeatCount > 1 && <div className="console-record-duplicate-number">{record.repeatCount}</div>}
            <div className="console-record-content">{this._renderContent()}</div>
          </div>
          {sourceLabel}
          {renderedTimestamp}
