...
export default class ConsoleView extends React.Component<Props, State> {
  ...
  componentDidMount(): void {
    this._disposables.add(
      ...
      atom.commands.add(nullthrows(this._consoleScrollPaneEl), "atom-ide:filter", () => this._focusFilter())
    )
  }
  ...
  componentDidUpdate(prevProps: Props): void {
    ...
    if (this._isScrolledNearBottom && recordsChanged(prevProps.records, this.props.records)) {
      this._startScrollToBottom()
    }
  }
        ...
        */}
        <div className="console-body" id={"console-font-size-" + this._id}>
          ...
          >
            ...
            />
            <NewMessagesNotification visible={this.state.unseenMessages} onClick={this._startScrollToBottom} />
          </div>
          {this._renderPrompt()}
        </div>
      </div>
  ...
  _startScrollToBottom = (): void => {
    if (!this._continuouslyScrollToBottom) {
      ...
      this._scrollingThrottle = Observable.timer(MAXIMUM_SCROLLING_TIME).subscribe(() => {
        this._stopScrollToBottom()
      })
    }
